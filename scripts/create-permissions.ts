...
import { HTTPMethod } from 'src/common/constants/method.constant';
...
async function bootstrap() {
  ...
  const commonRoleRepository = app.get(CommonRoleRepository);
  const server = app.getHttpAdapter().getInstance();
  const router = server.router;
  const adminRoleId = await commonRoleRepository.getAdminRoleId();
  console.log('ROLE ID ADMIN', adminRoleId);
  if (!adminRoleId) throw new Error('Admin role ID is missing');
  ...
  await Promise.all([
    updateRole(adminPermissionIds, ROLE_NAME.ADMIN),
    updateRole(sellerPermissionIds, ROLE_NAME.SELLER),
    updateRole(clientPermissionIds, ROLE_NAME.CLIENT),
  ]);
  process.exit(0);
}
