...
let TwoFactorService = class TwoFactorService {
    ...
    constructor(commonUserRepository) {
        this.commonUserRepository = commonUserRepository;
    }
    createTOTP(email, secret) {
        return new OTPAuth.TOTP({
            issuer: validate_env_1.default.APP_NAME,
            label: email,
            algorithm: 'SHA1',
            digits: 6,
            period: 30,
    ...
    async enableTwoFaForUser(userId, secretBase32) {
        return await this.commonUserRepository.update({ id: userId }, { totpSecret: secretBase32 });
    }
    async disableTwoFaForUser(userId) {
        return await this.commonUserRepository.update({ id: userId }, { totpSecret: null });
    }
};
